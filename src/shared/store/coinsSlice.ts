...
import { AllCoinsProps, CoinType, FetchStatus } from "../types/coins";
...
export const fetchCoinsMarket = createAsyncThunk<
  ...
  string,
  { rejectValue: string }
>("coins/fetchCoinsMarket", async (currency, { rejectWithValue }) => {
  try {
    const { data } = await fetchCoinData(
      `/coins/markets?vs_currency=${currency}&order=market_cap_desc&per_page=250&page=1&sparkline=true&price_change_percentage=1h%2C24h%2C7d`
    );
    return data;
  } catch (err) {
